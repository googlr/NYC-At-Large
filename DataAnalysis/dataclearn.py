...
# 40.498060701
min_lat = sqlContext.sql("SELECT MIN(Latitude)  FROM crime WHERE Latitude != ''")
min_lat.show()
# 40.912723396
max_lat = sqlContext.sql("SELECT MAX(Latitude)  FROM crime WHERE Latitude != ''")
max_lat.show()
# -73.700315857
min_lon = sqlContext.sql("SELECT MIN(Longitude)  FROM crime WHERE Longitude != ''")
min_lon.show()
# -74.255075543
max_lon = sqlContext.sql("SELECT MAX(Longitude)  FROM crime WHERE Longitude != ''")
max_lon.show()

# borough count
borough_count =  sqlContext.sql("SELECT BORO_NM, count(*)  FROM crime GROUP BY BORO_NM")
borough_count.show()
# borough_count.toPandas().to_csv('borough_count.csv')
